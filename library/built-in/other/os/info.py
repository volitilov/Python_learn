...
path.islink('file.txt')
# возвращает True, если объект является символической ссылкой, и False 
# - в противном случае. Если символические ссылки не поддерживаются, 
...
path.normcase(path)
# преобразует заданный путь к каталоrу к виду, подходяшему для использования 
# в текущей операционной системе. В Windows преобразует все прямые слеши в 
# обратные. Также во всех системах приводит все буквы пути к нижнему регист­ру.
...
mkdir(path, права_доступа)
# создает новый каталог с правами досту­па, указанными во втором параметре. 
# Права доступа задаются восьмеричным числом (значение по умолчанию Оо777).

makedirs('imgs2', exist_ok=True)
# создаём дерикторию придворительно проверяя её присутствие

rmdir('path')
# удаляет пустой каталог. Если в каталоге есть файлы или ука­занный каталог 
# не существует, возбуждается исключение - подкласс класса OSError.
...
# функцией, зависит преобразование относительного пути в абсолютный. Кроме 
# того, важно помнить, что текущим рабочим каталогом будет каталог, из 
# которого запускается файл, а не каталог с исполняемым файлом.
...
# В качестве значения функция walk() возвращает объект. На каждой итерации 
# через этот объект доступен кортеж из трех элементов: текущего каталога, 
# списка каталогов и спи­ска файлов, находящихся в нем. Если произвести 
# изменения в списке каталогов во время выполнения, это позволит изменить 
# порядок обхода вложенных каталогов.
# Необязательный параметр topdown задает последовательность обхода каталогоа.
