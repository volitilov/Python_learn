...
th.settrace(func)
# установливает функцию трассировки для всех потоков, 
# запущенных из модуля потоковой передачи. Функция func будет 
# передана sys.settrace() для каждого потока, прежде чем 
# вызывается метод run()

th.setprofile(func)
# задаёт функцию профиля для всех потоков, запущенных из модуля 
...
# потоков. Необязательный аргумент размера определяет размер 
# стека, который будет использоваться для созданных впоследствии 
# потоков, и должен быть либо 0 (использует платформу или 
# настроен по умолчанию), либо положительное целое значение не 
# менее 32768 (32 KiB). Если размер не указан, используется 0. 
# Если изменение размера стека нитей не поддерживается, возникает 
...
# поток, который вызывает этот метод, приостанавливается, 
# ожидая завершения потока, чей метод вызван. Параметр timeout 
# (число с плавающей точкой) позволяет указать время ожидания 
# (в секундах), по истечении которого приостановленный поток 
# продолжает свою работу независимо от завершения потока, чей 
# метод join был вызван. Вызывать join() некоторого потока 
# можно много раз. Поток не может вызвать метод join() самого 
# себя. Также нельзя ожидать завершения еще не запущенного 
# потока.
...
# Идентификаторы потоков могут быть переработаны, когда поток 
# выходит и создается другой поток. Идентификатор доступен даже 
# после выхода потока.
...
th_obj.daemonic
# возвращает логическое значение, указывающее, является ли этот 
# поток потоком демона(True) или нет(False). Это необходимо установить 
# перед вызовом start(), иначе будет поднято исключение RuntimeError. 
# Его начальное значение наследуется от создающего потока; основной 
# поток не является потоком демона, и поэтому все потоки, созданные в 
# основном потоке по умолчанию, равны daemon = False. Вся программа 
# Python завершается, когда не осталось живых ничейных нитей
...
th_obj.setDaemon(daemonic)
# устанавливает признак daemonic того, что поток является 
# демоном
...
# Примитивный замок - это примитив синхронизации, который не 
# принадлежит определенному потоку при блокировке. Примитивный 
# замок находится в одном из двух состояний, «заблокирован» или 
# «разблокирован». Он создается в незаблокированном состоянии. 
...
# Блокировка реентера - это примитив синхронизации, который может 
# быть получен несколько раз одним и тем же потоком. Внутри он 
# использует понятия «владение нитью» и «уровень рекурсии» в 
# дополнение к заблокированному/разблокированному состоянию, 
# используемому примитивными замками. В заблокированном состоянии 
# некоторый поток владеет блокировкой; в незаблокированном 
# состоянии ни одна из них не владеет им.
...
# Переменная условия всегда связана с каким-то замком; это может 
# быть передано или один будет создан по умолчанию. Передача 
# одного из них полезна, когда несколько переменных условия 
# должны иметь одну и ту же блокировку. Замок является частью 
# объекта условия: вам не нужно отслеживать его отдельно.
...
# Этот класс обеспечивает простой примитив синхронизации для 
# использования фиксированное число потоков, которым нужно ждать друг 
# друга. Каждый из потоков пытается передать барьер, вызвав метод 
# wait() и заблокирует, пока все потоки не вызовут вызовы wait(). В 
# этот момент потоки освобождаются одновременно.

bar.wait(timeout=None)
# пройти через барьер. Когда все потоки, связанные с барьером, 
# вызвали эту функцию, все они выпущены одновременно. Если задан 
# тайм-аут, он используется в предпочтении любому, который был 
# предоставлен конструктору класса.
