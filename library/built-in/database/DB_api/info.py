...
# Согласно спецификации DB-API 2.0, последовательность работы с базой 
# данных выглядит следующим образом:

	1. # Производится подключение к базе данных с помощью функции 
		# connect(). Функция воз­вращает объект соединения, с помощью 
		# которого осуществляется дальнейшая работа с базой данных.

	2. # Создается объект-курсор.

	3. # Выполняются SQL-зanpocы и обрабатываются результаты. Перед 
		# выполнением первого запроса, который изменяет записи (INSERT, 
		# REPLACE, UPDATE и DELETE), автоматически за­пускается транзакция.

	4. # Завершается транзакция или отменяются все изменения в рамках 
		# транзакции.
